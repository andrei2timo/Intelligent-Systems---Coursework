%   Two witnesses that I (id 19000915) I ran the program and considered the output of the code. 
%   Demonstrated to Yu(Roland) Thong (student id 18045346) Thursday 11-March-2021  
%   Demonstrated to Harry Tidball (student id 15015043) Wednesday 10-March-2021

%    I ran  the program GA_2.m (a Matlab notebook) and established %TA
%    that it contains five distinct activities, namely: %TA
%       a) generating a population of chromosomes, %TA
%       b) obtaining decoded integers from the chromosome strings, %TA
%       c) displaying initial locations of the chromosomes on the surface of the "peak" function., %TA
%       d) Creating a new population of chromosomes, %TA
%       e) Replacing the entire parent chromosome population with a new
%       one. %TA
%    Observations on running the algorithm, it solves a Genetic Algorithm in a  reasonable time. %TA
%    Whilst trying to improve the runtime, experimenting with the following  parameters "ngener=45,ngenes=8,genesPc=0.99 and Pm=0.001" %TA
%    but on each occasion I corrupted the algorithm by changing the value of the parameters because the results will differ. %TA
%    Further observations are Crossover is a process in which members of the last population %TA
%    are mated at random in the mating pool. So, a pair of offsprings is generated, %TA
%    combining elements from two parents (members), which hopefully have improved fitness values. %TA
%    Mutation is the occasional (with small probability) random alteration of the value of a string %TA
%    position. In fact, mutation is a process of random walk through the coded parameter space. %TA
%    Its purpose is to ensure that important information contained within strings may not be lost prematurely. %TA
%    I believe that would be useful direction to explore it because %TA
%    genetic algorithms are randomized search algorithms that have been developed %TA
%    in an effort to imitate the mechanics of natural selection and natural genetics. %TA
%    Genetic algorithms operate on string structures, like biological structures, %TA
%    which are evolving in time according to the rule of survival of the fittest %TA
%    by using a randomized yet structured information exchange. %TA
%    Thus, in every generation, a new set of strings is created, using parts %TA
%    of the fittest members of the old set. %TA
%
...
ngenes=8;  % Number of genes in a chromosome: each variable is represented by (ngenes/2) genes
Pc=0.99;     % Crossover probability
...
ngener=45; % Number of generations
